...
def test_option_bounds_are_configurable_for_contract_heads():
    single = dict(ROW, options=[{"id": "only", "description": "Only candidate."}])
    with pytest.raises(ValueError, match="2-16"):
        validate_row(single)
    validate_row(single, min_options=1)
    wide = dict(ROW, options=[{"id": f"c{index}", "description": "Candidate."} for index in range(20)])
    with pytest.raises(ValueError, match="2-16"):
        validate_row(wide)
    validate_row(wide, min_options=1, max_options=20)

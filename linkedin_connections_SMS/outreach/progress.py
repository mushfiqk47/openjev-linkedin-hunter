"""Progress tracking: the outreach funnel (total / messaged / remaining).

Files in data/:
  - total_connections.txt : one integer, the LinkedIn connection total
  - progress.txt          : human/AI-readable snapshot (rewritten each run)
"""
import datetime
...
# Read the "N connections" figure from the connections page header.
# NOTE: no backslash escapes anywhere — this text is exec'd as Python by
# browser-use and then embedded in JS, so escapes would be consumed twice.
COUNT_JS = """
import json, time
...
def fetch_total_connections(timeout=None):
    """Return the total connection count as an int, or None on failure."""
...
def read_saved_total():
    """Return the previously saved total connection count, or None."""
    try:
...
def refresh_progress(total_connections=None):
    """Recompute and save progress.txt from the ledger. When
    total_connections is None, the previously saved total is reused.
    Returns a dict with the current progress numbers."""
...
def run(timeout=None):
    """CLI: fetch the live total, refresh and print the funnel."""

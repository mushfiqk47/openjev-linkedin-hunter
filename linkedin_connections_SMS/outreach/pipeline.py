"""Pure Autonomous Outreach Pipeline.

Orchestrates the self-updating connection lifecycle:
  1. Registry state check: instant O(1) skips for any previously messaged/skipped contact.
  2. Network delta sync: automatically indexes newly added connections.
  3. SemIf decision engine: scores relevance and classifies archetype (recruiter/founder/peer/general).
  4. One-time first-contact check: verifies conversation state only for brand new leads.
  5. Personalized dispatch & 3-point delivery verification.
  6. Real-time registry persistence & progress funnel update.
"""
...
def run_pipeline(
    ...
    """Run the pure, self-updating LinkedIn outreach pipeline."""
    ...
    # 1. Delta Sync: Fetch newly added connections, skip already messaged in 0ms
    ...
    # 2. SemIf Evaluation: Filter low relevance and classify archetypes
    ...
    for i, contact in enumerate(to_process):
        ...
        # SemIf: Archetype & Relevance evaluation
        ...
        # Low-relevance filter: permanently record SKIPPED so they are never revisited
        ...
        # One-time live thread check (only for brand new unsent leads)

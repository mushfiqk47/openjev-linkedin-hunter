"""Network Delta Synchronization Engine.

Continuously discovers 1st-degree connections from LinkedIn and syncs with
ConnectionsRegistry:
  - Skips already messaged/skipped contacts in O(1) time without loading profiles.
  - Automatically identifies newly added connections and queues them as UNSENT.
  - Preserves network state across dynamic connection additions and removals.
"""
...
def sync_network(
    ...
    """Sweep LinkedIn 1st-degree connections and sync state into the registry.

    Returns the list of newly added or pending unsent contacts ready for outreach.
    """
    ...
    # 1. Sweep Recent Connections Page
    if not search_only:
        ...
        for line in out.splitlines():
            ...
            for link in links:
                ...
                # O(1) state check: already messaged or skipped?
    ...
    # 2. Paginated 1st-Degree People Search for Bulk / Older Connections
    ...
    if len(unsent_pool) < target_new:
        ...
        for page in range(start_page, start_page + max_search_pages):
            ...
            for item in items:
                ...
                # O(1) Instant Lookup: If already SENT or SKIPPED, skip in 0ms!
